...
def _find_quarto() -> str | None:
    ...
    # 1. Config override: ~/.pdf2md/config.json → quarto_path
    ...
    candidates = [
        Path.home() / "quarto" / "bin" / "quarto",         # Linux/macOS manual
        Path.home() / "opt" / "quarto" / "bin" / "quarto",  # Linux
        Path("/usr/local/bin/quarto"),                         # Homebrew, Linux
        Path("/opt/quarto/bin/quarto"),                        # Linux
        Path("C:/Program Files/Quarto/bin/quarto.exe"),        # Windows
        Path.home() / "AppData" / "Local" / "Programs" / "Quarto" / "bin" / "quarto.exe",  # Windows user
    ]
    for c in candidates:
        if c.exists():
            return str(c)

...
`FIG_n`. Its job is to transcribe the document to Markdown Markdown (.qmd) and
...
it out into a Markdown table-figure div, so it renders ABOVE the table, styled exactly
...
a single GitHub-Flavored Markdown (.md) document.
...
- SIMPLE = a plain rectangular grid: one header row, every body row has exactly
  ...
  Transcribe these as GitHub pipe tables (| col | col |). Add the caption with Markdown
...
This single block renders in BOTH HTML and Typst-PDF: Markdown parses HTML tables
...
Convert {{FILENAME}} to a Markdown .qmd document following your instructions.
